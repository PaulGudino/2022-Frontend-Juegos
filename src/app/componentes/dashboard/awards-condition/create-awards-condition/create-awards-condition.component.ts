...
export class CreateAwardsConditionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.minDatefin = new Date(currentYear, currentMonth, currentDay + 7);
    this.finishDate = new Date(currentYear, currentMonth, currentDay + 7);
  }
  async changetime(){
    ...
    if (this.form.valid) {
      ...
      this.dialogService.confirmed().subscribe(confirmed => {
        if (confirmed) {
          ...
          formData.append('amount', this.form.get('initial_stock')?.value);
